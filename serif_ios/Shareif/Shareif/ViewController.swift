...
class ViewController: UIViewController, AVCaptureMetadataOutputObjectsDelegate {
    ...
    func found(code: String) {
        print("[SHAREIF] This is code - \(code)")

        let defaults = UserDefaults(suiteName: "group.jmaric.Shareif.ShareifShare")
        let data = defaults?.object(forKey: "data")
        
        if let data = data {
            print("[SHAREIF] Successfully fetched data binary")
            print("[SHAREIF] Here goes data send to server")
        } else {
            print("[SHAREIF] Error fetching data binary")
        }
    }
